# quantix masternode install script
# Edited by Robbowz
VERSION="0.3"
NODEPORT='26668'
RPCPORT='12455'
...
declare -r SCRIPT_LOGFILE="/tmp/quantix_node_${DATE_STAMP}_out.log"
declare -r SCRIPTPATH=$( cd $(dirname ${BASH_SOURCE[0]}) > /dev/null; pwd -P )
declare -r WANIP=$(dig +short myip.opendns.com @resolver1.opendns.com)


function print_greeting() {
	echo -e "[0;35m quantix masternode install script[0m\n"
}
...
function swaphack() {
	...
	free -h
	rm -f /var/quantix_node_swap.img
	touch /var/quantix_node_swap.img
	dd if=/dev/zero of=/var/quantix_node_swap.img bs=1024k count=2000 &>> ${SCRIPT_LOGFILE}
	chmod 0600 /var/quantix_node_swap.img
	mkswap /var/quantix_node_swap.img &>> ${SCRIPT_LOGFILE}
	free -h
...
function configure_firewall() {
	...
	ufw allow 26668/tcp			&>> ${SCRIPT_LOGFILE}
	ufw allow 12455/tcp			&>> ${SCRIPT_LOGFILE}
...
function configure_masternode() {
	...
	/root/quantix/quantix-cli stop
	echo "It's okay."
	sleep 10
	echo -e "rpcuser=quantixuser\nrpcpassword=${PASSWORD}\nrpcport=${RPCPORT}\nport=${NODEPORT}\nexternalip=${WANIP}\nlisten=1\nmaxconnections=250" >> ${conffile}
	echo ""
	echo -e "[0;35m==================================================================[0m"
	...
	echo -e "[0;35m==================================================================[0m"
	echo ""
	/root/quantix/quantixd -daemon
	echo "2 MINUTES LEFT"
	sleep 60
	echo "1 MINUTE LEFT"
	sleep 60
	masternodekey=$(/root/quantix/quantix-cli masternode genkey)
	/root/quantix/quantix-cli stop
	sleep 20
...
#Setting auto start cron job for quantixd
cronjob="@reboot sleep 30 && /root/quantix/quantixd"
crontab -l > tempcron
...
echo "All done!"
cd ~/
sudo rm /root/quantix_ubuntu_install.sh
